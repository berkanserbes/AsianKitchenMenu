...
for(let i=0; i < categories.length; i++) {
  ...
  btnDOM.append(newButton)
}

// Adding functions to buttons
function buttonClick() {
  let filtering = menu.filter(item => item.category == this.getAttribute("data"))
  this.getAttribute("data") == "All" ? filtering = menu : "";
  let result = filtering.map((itemResult) => {
    return `
      <div class = "menu-items col-lg-6 col-sm-12">
        ...
        <div class = "menu-info">
          ...
          <div class= "menu-text"> ${itemResult.desc}
          </div>
        </div>
      </div>
    `;
  ...
  sectionDOM.innerHTML = result;
}

let showPage = menu.map((itemResult) => {
  return `
  <div class = "menu-items col-lg-6 col-sm-12">
    <img src="${itemResult.img}" alt="" class = "photo">
    <div class = "menu-info">
